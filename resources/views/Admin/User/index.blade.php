...
<div class="page-container">
	...
	<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$val->phone}}</td>
				<td>{{$val->email}}</td>
				<td><img src="./uploads/Face/{{$val->face}}" width="50px"></td>
				<td>{{$val->created_at}}</td>
				<td>{{$val->updated_at}}</td>
				<td class="td-manage">
					<form action="/adminuser/edit" method="get">
					<button title="修改" class="ml-5" style="text-decoration:none;float:right;margin-right:15px;"><i class="Hui-iconfont">&#xe6df;</i>
					</button> 
				</form>
					<form action="/adminuser/{{$val->id}}" method="post">
					{{method_field("DELETE")}}
					{{csrf_field()}}
					<button title="删除" href="/adminuser/{{$val->id}}" onclick="user_del(this,'1')" class="ml-5" 	style="text-decoration:none;float:right;"><i class="Hui-iconfont">&#xe6e2;</i></button>
