		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			@foreach($data as $val)
				<tr class="text-c">
					...
					<td>{{$val->staffld}}</td>
					<td><img src="{{$val->face}}" width="100px"></td>
					<td>{!!$val->content!!}</td>
					<td class="td-status">{{$val->created_at}}</td>
					<td id="jb">{{$val->isshow}}</td>
					<td class="f-14 td-manage">
						<!-- <input type="hidden" name="{{$val->id}}" id="aa"> -->
						<a style="text-decoration:none" name="{{$val->id}}" href="javascript:void(0);" title="下架" class="xia"><i class="Hui-iconfont">&#xe6de;</i>
						</a> 		
						<a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','article-add.html','10001')" href="/article/{{$val->id}}/edit" title="编辑"><i class="Hui-iconfont">&#xe6df;</i>
						</a> 
...
<script>
	$(".xia").click(function(){
		oc = $(this);
		id=oc.attr("name");
		 $.get("/xiajia",{id:id},function(a){
			oc.parents("td").prev('td').html(a.isshow);
		 },"json");
	});
</script>
@endsection
